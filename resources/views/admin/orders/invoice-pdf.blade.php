...
<div class="container">
  ...
  <div class="header">
    <div class="flex">
      <div>
        <h1>INVOICE</h1>
        <p>Invoice Number: <strong>{{ $order->code }}</strong></p>
      </div>

      <div>
        <h2 style="margin:0; font-size:20px; font-weight:bold;">{{ setting('site_name') }}</h2>
        <p style="margin:0; font-size:12px;">{{ setting('site_description') }}</p>
      </div>
    </div>
  </div>
  ...
  <div class="flex box">
    <div style="width:48%;">
      ...
      <p>{{ setting('site_name') }}<br>
        {{ setting('address') }}<br>
        Phone: {{ setting('phone_contact') }}<br>
        Email: {{ setting('email_contact') }}<br>
      </p>
    </div>
  ...
  <div class="total-box">
    ...
    {{-- @if($order->discount > 0)
      <div>
        <span>Discount:</span>
        <strong style="color:red;">-Rp{{ number_format($order->discount,0,',','.') }}</strong>
      </div>
    @endif --}}
    ...
    <div style="border-top:1px solid #aaa; margin-top:8px; padding-top:8px; font-size:14px;">
      ...
      <strong style="color:#1e3a8a;">Rp{{ number_format($order->grand_total,0,',','.') }}</strong>
    </div>
  </div>
  ...
  @if($order->payment_status === 'paid')
    <div class="paid-box">
      ...
      {{-- @if($order->payment_date)
        <small>Paid on {{ $order->payment_date->format('d M Y, H:i') }}</small>
      @endif --}}
    </div>
  @endif
