  ...
  <div class="col-span-12">
    ...
    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/3">
      <div class="px-5 py-4 sm:px-6 sm:py-5 flex items-center justify-between">
        ...
        <a href="{{ route('profile.addresses.index') }}" class="text-sm text-blue-600 hover:underline dark:text-blue-400">← Back</a>
      </div>

      <form action="{{ route('profile.addresses.update', $address) }}" method="POST" class="border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800 space-y-6">
        @csrf
        @method('PUT')
        ...
        <div>
          ...
          <select name="label" class="w-full border rounded-lg px-3 py-2 dark:bg-gray-900 dark:border-gray-700 dark:text-white">
            <option @selected(old('label', $address->label) == 'Home') value="Home">Home</option>
            <option @selected(old('label', $address->label) == 'Work') value="Work">Work</option>
            <option @selected(old('label', $address->label) == 'Office') value="Office">Office</option>
            <option @selected(old('label', $address->label) == 'Other') value="Other">Other</option>
          </select>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" id="latitude" name="latitude"
              value="{{ old('latitude', $address->latitude) }}" readonly
          ...
          <div>
            ...
            <input type="text" id="longitude" name="longitude"
              value="{{ old('longitude', $address->longitude) }}" readonly
              class="w-full border rounded-lg px-3 py-2 bg-gray-100 text-gray-600 cursor-not-allowed dark:bg-gray-800 dark:border-gray-700 dark:text-gray-400">
          </div>
        </div>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const initialLat = parseFloat('{{ $address->latitude }}') || -8.65;
  const initialLng = parseFloat('{{ $address->longitude }}') || 115.2167;
