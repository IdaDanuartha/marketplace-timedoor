...
<?php $__env->startSection('content'); ?>
    <div class="max-w-6xl mx-auto py-8">
        ...
        <?php if(session('success')): ?>
            <div class="p-3 bg-green-100 border border-green-300 text-green-800 rounded-lg mb-4"><?php echo e(session('success')); ?>
        ...
        <div class="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            <?php $__empty_1 = true; $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <div
                    class="border rounded-lg bg-white dark:bg-gray-900 dark:border-gray-800 p-4 flex flex-col justify-between relative">
                    
                    <form action="<?php echo e(route('shop.wishlist.toggle', $product)); ?>" method="POST"
                        class="absolute top-3 right-3"><?php echo csrf_field(); ?>
                        <?php
                            $isWished =
                                Auth::check() && Auth::user()->customer
                        ...
                        <button type="submit" title="Add to Wishlist">
                            <?php if($isWished): ?>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor"
                                    ...
                                    <path
                                        d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5a5.5 5.5 0 0111 0 5.5 5.5 0 0111 0c0 3.78-3.4 6.86-8.55 11.54L12 21.35z" />
                                </svg>
                            <?php else: ?>
                    ...
                    <a href="<?php echo e(route('shop.products.show', $product)); ?>">
                        ...
                        <p class="text-blue-600 font-semibold mt-2">Rp <?php echo e(number_format($product->price, 0, ',', '.')); ?>
                        ...
                        <div class="flex items-center gap-1 mt-1">
                            ...
                            <?php for($i = 1; $i <= 5; $i++): ?>
                                <svg xmlns="http://www.w3.org/2000/svg" fill="<?php echo e($i <= $avg ? 'currentColor' : 'none'); ?>"
                                    class="w-4 h-4 <?php echo e($i <= $avg ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'); ?>"
                            ...
                            <?php endfor; ?>
                            <span class="text-xs text-gray-500">(<?php echo e($avg); ?>/5)</span>
                        </div>
                    </a>
                    ...
                    <div class="mt-4 flex gap-2">
                        <form action="<?php echo e(route('shop.cart.add', $product)); ?>" method="POST"><?php echo csrf_field(); ?>
                            <button
                                class="px-3 py-2 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 w-full">Add
                                to Cart</button>
                        </form>
                        <form action="<?php echo e(route('shop.cart.buyNow', $product)); ?>" method="POST"><?php echo csrf_field(); ?>
                            <button class="px-3 py-2 bg-blue-600 text-white text-sm rounded-lg hover:bg-blue-700 w-full">Buy
                                Now</button>
                        </form>
                    </div>
                </div>
