      ...
      <div class="border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800 space-y-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 gap-4 text-sm">
          ...
          <div class="mb-4">
            ...
            <?php elseif(auth()->user()?->vendor): ?>
              <form method="POST" action="<?php echo e(route('orders.updateStatus', $order)); ?>" class="flex items-center gap-2">
                <?php echo csrf_field(); ?>
                <?php echo method_field('PUT'); ?>
                <select name="status" 
                        class="border rounded-md text-sm px-2 py-1 dark:bg-gray-900 dark:border-gray-700 dark:text-white"
                        onchange="this.form.submit()">
                  <?php $__currentLoopData = \App\Enum\OrderStatus::cases(); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $status): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    
                    <?php if(!in_array($status->value, [\App\Enum\OrderStatus::CANCELED->value])): ?>
                      <option value="<?php echo e($status->value); ?>" <?php echo e($order->status === $status ? 'selected' : ''); ?>>
